...
var calendar = function(){
    ...
    var current_resource = function(){
    	var matchedResource = window.location.href.match(/cars\/(\d+)/);

      if(matchedResource != null) {
        return matchedResource[1];
      }      
    };
    ...
    $('#calendar').fullCalendar({
        header: {
			...
			eventSources: [{  
    		url: '/cars/'+current_resource()+'/bookings/',  
   		}],
    ...
    eventDrop: function(booking) {
      var length = (booking.end-booking.start)/(3600000);
        ...
        function updateEvent(booking) {
              $.ajax(
                '/cars/'+current_resource()+'/bookings/'+booking.id,
                { 'type': 'PATCH',
    ...
    ,
      ...
      if (window.location.href.match(/new/)) {
        if(today_or_later()) {
          ...
          jQuery.post(
            '/cars/'+current_resource()+'/bookings',
